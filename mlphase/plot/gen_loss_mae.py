...
def mae_loss_(r_true, r_pred):
    idx = [0, 1, 3, 4, 6, 7, 9, 10, 11]
    mae = r_pred[:, idx] - r_true[:, idx]
    mae = np.abs(mae).mean(axis=1)
    return mae


def extract_loss_mae(RESULT_DIR, model):
    mask = 0
    sample_ratio = 1.0
    ...
    for i in range(1, 6):
        try:
            ...
            loss.append(mae_loss_(yr_test, yr_pred))
        except:
            continue
...
def gen_loss_mae(RESULT_DIR, PLOT_DIR, COLORS, format="png"):
    x_vals = np.linspace(-4, 0, 100)
    bins = np.linspace(-4, 0, 50)
    alpha = 0.2

    loss_base = extract_loss_mae(RESULT_DIR, "base")
    loss_pi = extract_loss_mae(RESULT_DIR, "softbase")
    loss_pir = extract_loss_mae(RESULT_DIR, "softpir")
    ...
    ax.format(
        xlabel=r"log$_{10}$ $\mathcal{L}_\mathrm{MAE}$",
        ylabel="Density",
        xlabelsize=13,
        ylabelsize=13,
        xticklabelsize=12,
    ...
    out_file = os.path.join(PLOT_DIR, f"loss_mae.{format}")
