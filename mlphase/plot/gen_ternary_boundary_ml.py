...
def gen_ternary_boundary_ml(
    ...
    """
    ML-predicted ternary phase-coexistence curves.
    """
    ...
    if three_phase:
        ...
        if avg_three_phase:
            ...
            ax.plot(
                tri_1.mean(axis=-1)[[0, 1, 2, 0]],
                tri_2.mean(axis=-1)[[0, 1, 2, 0]],
                tri_3.mean(axis=-1)[[0, 1, 2, 0]],
                c=COLORS[0],
                lw=2,
                linestyle="--",
            )
    ...
    out_name = f"{out_names[1]}-{out_names[2]}-{out_names[7]}-{out_names[9]}-{out_names[-1]}"
    # out_file = os.path.join(PLOT_DIR, f"{out_name}_ml.{format}")
    out_file = os.path.join(PLOT_DIR, f"{out_name}_ml_new.{format}")
